...
def login():
    if request.method == 'POST':
        ...
        student = students.find_one({'email': email, 'password': hash_password(password)})
        if student:
...
def leaderboard():
    ...
    for submission in completed_submissions:
        student_id = submission.get('student_id', 'unknown_id')
        if 'student_email' not in submission:
            print(f"Warning: Missing student_email in submission: {submission}")

        student_stats[student_id] = {
            'name': submission.get('student_name', 'Unknown'),
            'email': submission.get('student_email', 'N/A'),  # Safe access
            'total_score': 0,
            'total_tests': 0,
